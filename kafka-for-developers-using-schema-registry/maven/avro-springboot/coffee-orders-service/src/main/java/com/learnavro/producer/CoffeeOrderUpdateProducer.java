...
public class CoffeeOrderUpdateProducer {
    KafkaTemplate<String, CoffeeUpdateEvent> kafkaTemplate;// refactor -> typeMigration -> select current file -> In migrate type of field modify from CoffeeOrder to CoffeeUpdateEvent
    ...
    private void handleSuccess(CoffeeUpdateEvent coffeeOrder, SendResult<String, CoffeeUpdateEvent> result) {
        log.info("Message Sent SuccessFully for the CoffeeUpdateEvent, key : {} and the value is {} , partition is {}", coffeeOrder.getId(), coffeeOrder, result.getRecordMetadata().partition());
    }
}
